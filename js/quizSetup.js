...
function generateFindNextPreviousQuestion(direction) {
    ...
    const wrongOptions = allAyahsInScope
        .filter(a => a.numberInSurah !== correctAyah.numberInSurah && a.numberInSurah !== questionAyah.numberInSurah)
    ...
    return { 
        type: 'find_text', 
        title: `استمع للآية، ثم اختر الآية ال${isNext ? 'تالية' : 'سابقة'} لها:`, 
        audio: questionAyah.audio, 
        options: [correctAyah, ...wrongOptions].sort(() => 0.5 - Math.random()), 
...
function generateFindNextPreviousOrderQuestion(direction) {
    ...
    const correctIndex = shuffledSequence.findIndex(a => a.numberInSurah === correctAyah.numberInSurah);

    return { 
        type: 'find_order', 
        title: `استمع للآية المرجعية، ثم استمع للمقطع، واختر ترتيب الآية ال${isNext ? 'تالية' : 'سابقة'}.`, 
        referenceAudio: questionAyah.audio, 
        audioSequence: shuffledSequence.map(a => a.audio), 
...
function generateOrderScrambledQuestion() {
    ...
    return { 
        type: 'order_scrambled', 
        title: 'استمع للمقطع الصوتي، ثم قم بترتيب الآيات حسب سماعك لها.', 
        audioSequence: scrambledSequence.map(a => a.audio), 
        correctAnswer: correctSequence.map(a => scrambledSequence.indexOf(a) + 1).join('-') 
    };
}
...
function generateFindIntruderQuestion(direction) {
    ...
    return { 
        type: 'find_intruder', 
        title: isOut ? 'اكتشف الآية التي ليست من النطاق المحدد.' : 'اكتشف الآية الوحيدة التي من النطاق المحدد.', 
        audioSequence: shuffledOptions.map(a => a.audio), 
...
// --- Main Quiz Preparation Function ---
...
export function prepareQuiz() {
    ...
    while (newQuizQuestions.length < numQuestions && attempts < maxAttempts) {
